...
export class ChatBoxComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    let accountNumberPattern = /\d{9,14}/g;
    console.log(this.chat.message.match(accountNumberPattern));
    let tranferDetects = this.chat.message.match(accountNumberPattern);
    if (tranferDetects && tranferDetects.length > 0) {
      this.transferAccountNumber = tranferDetects[0];
    }
    if (
      (this.transferAccountNumber || this.transferBankName) &&
      this.chat.sender != this.current_user_id
    ) {
    ...
    let shortNames = banks.map((bank) =>
      bank.shortName.toLowerCase().replace(/ /g, "")
    );
    for (let i = 0; i < shortNames.length; i++) {
      if (this.chat.message.search(shortNames[i]) !== -1) {
        this.transferBankName = shortNames[i];
        break;
      }
    }

    let oneMil = ["củ", "triệu", "chai"];

    for (let i = 0; i < oneMil.length; i++) {
      if (this.chat.message.search(oneMil[i]) !== -1) {
        this.transferBankName = shortNames[i];
        for (let j = this.chat.message.search(oneMil[i]) - 1; j >= 0; j--) {
          if (this.chat[j] == " ") {
            this.transferAmount = this.chat.slice(
              j + 1,
              this.chat.message.search(oneMil[i])
            );
    ...
    let oneHoundredThousand = ["lít", "lốp"];

    let oneThousand = ["k", "nghìn", "ngàn"];
  }
}
